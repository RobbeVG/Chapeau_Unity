...
namespace Seacore
{
    public class DiceManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (!item.Value.IsOutside)
                {
                    //Make sure outline gets updated once so that material is adjusted
                    item.Value.Outline.enabled = false;
                }
            }

            if (Physics.Raycast(ray, out hit)) // All different objects are not raycastable (See: layers)
            {
                Transform selection = hit.transform;

                //Possible to save gameobjects instead of dice for improvement
                    
                if (selection.gameObject.TryGetComponent<Die>(out Die dieComponent))
                {
                   
                    DieInfo dieInfo = _dieContainers[dieComponent];
                    dieInfo.Outline.enabled = true;
                    if (Input.GetMouseButtonDown(0))
                    {
                        dieInfo.IsOutside = !dieInfo.IsOutside;
                        if (dieInfo.IsOutside)
                            dieInfo.Outline.OutlineColor = Color.red;
                        else
                            dieInfo.Outline.OutlineColor = Color.white;
                    }
                }
            }
        ...
        public void Roll()
        {
            foreach (KeyValuePair<Die, DieInfo> dieInfoPair in _dieContainers)
            {
                ...
                physicalRoll.ChangeValue(info.Index, die.RolledValue);
            }
        }
    }
}
